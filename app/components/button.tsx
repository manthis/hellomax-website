...
export default function Button({ label, link }: ButtonProps) {
    ...
    return (
        <a
            href={link}
            className='mr-[20px] inline-block border-2 border-lightblue px-[20px] py-[10px] font-medium uppercase tracking-wider text-white no-underline duration-500 last:bg-lightblue last:text-dracula hover:bg-lightblue hover:text-dracula hover:shadow-lightblue'>
            {label}
        </a>
    );
}
